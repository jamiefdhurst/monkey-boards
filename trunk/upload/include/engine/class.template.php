...
class Template_Lite {
	...
	var $_version		= 'V1.70 Template Lite 15 April 2006  (c) 2005,2006 Mark Dickenson. All rights reserved. Released LGPL.';
...
//		$this->_error_level = error_reporting(E_ALL);
	...
	function config_load($file, $section_name = null, $var_name = null) {
		...
		{
			require_once(TEMPLATE_LITE_DIR . "internal/template.config_load.php");
		}
	}
	...
	function _fetch_compile($file) {
		...
		$f = @fopen($this->compile_dir.'c_'.$name, "w") or die('Fatal error: write permissions required in /include/engine/compiled');
		fwrite($f, $output);
		fclose($f);
